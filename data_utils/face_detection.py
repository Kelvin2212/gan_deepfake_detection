...
# def locate_face_in_imagefile(input_filepath=None, outfile_filepath=None):
#     capture = cv2.ImageCapture(input_filepath)
...
#         frames.append(frame)
#     create_image_from_images(frames, outfile_filepath, fps=org_fps, res=org_res)

def locate_face_in_image(input_image=None, outfile_filepath=None):
    detector = get_face_detector_model()
    ...
    if face_box is not None:
        for f in range(len(face_box)):
            fc = list(face_box[f])
            cv2.rectangle(frame, (fc[0], fc[1]),
                          (fc[2], fc[3]), (0, 255, 0), 4)
    cv2.imwrite(outfile_filepath, frame, [cv2.IMWRITE_JPEG_QUALITY, 100])

# def extract_landmarks_from_image(input_imagefile, out_dir, batch_size=32, detector=None, overwrite=False):
#     id = os.path.splitext(os.path.basename(input_imagefile))[0]
#     out_file = os.path.join(out_dir, "{}.json".format(id))

#     if not overwrite and os.path.isfile(out_file):
#         return

#     capture = cv2.ImageCapture(input_imagefile)
#     frames_num = int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
#     if detector is None:
#         detector = get_face_detector_model()
...
def extract_landmarks_from_image(input_image, out_dir, detector=None, overwrite=False):
    id = os.path.splitext(os.path.basename(input_image))[0]
    out_file = os.path.join(out_dir, "{}.json".format(id))
    ...
    frame_boxes, prob, keypoints = detector.detect(frame, landmarks=True)
    frame_boxes = frame_boxes.tolist() if frame_boxes is not None else None
    keypoints = keypoints.tolist() if keypoints is not None else None

    result = {0: (frame_boxes, keypoints)}
...
# def extract_landmarks_from_image_batch(input_filepath_list, out_dir):
#     os.makedirs(out_dir, exist_ok=True)
#     with multiprocessing.Pool(1) as pool:
#         jobs = []
#         results = []
#         for input_filepath in input_filepath_list:
#             jobs.append(pool.apply_async(extract_landmarks_from_image,
...
def extract_landmarks_from_images_batch(input_images_dir, out_dir):
    os.makedirs(out_dir, exist_ok=True)
    input_filepath_list = [os.path.join(input_images_dir, f) for f in os.listdir(
        input_images_dir) if f.endswith('.jpg')]

    with multiprocessing.Pool(1) as pool:
        jobs = []
        results = []
        for input_filepath in input_filepath_list:
            jobs.append(pool.apply_async(extract_landmarks_from_images,
                                         (input_filepath, out_dir,),
                                         )
                        )
...
# def draw_landmarks_on_image(in_imagefile, out_imagefile, landmarks_file):
#     capture = cv2.imageCapture(in_imagefile)
#     frames_num = int(capture.get(cv2.CAP_PROP_FRAME_COUNT))
#     org_height = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
#     org_width = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH))
#     org_res = (org_width, org_height)
...
#                     map(int, keypoints[0])), 2, (0, 155, 255), 2)
#                 cv2.circle(frame, tuple(
#                     map(int, keypoints[1])), 2, (0, 155, 255), 2)
#                 cv2.circle(frame, tuple(
#                     map(int, keypoints[2])), 2, (0, 155, 255), 2)
#                 cv2.circle(frame, tuple(
#                     map(int, keypoints[3])), 2, (0, 155, 255), 2)
#                 cv2.circle(frame, tuple(
#                     map(int, keypoints[4])), 2, (0, 155, 255), 2)

#         frames.append(frame)
#     create_image_from_images(frames, out_imagefile, fps=org_fps, res=org_res)
def draw_landmarks_on_image(input_image, output_image, landmarks_file):
    with open(landmarks_file, 'r') as jf:
        face_box_dict = json.load(jf)
    ...
    if image is not None:
        if '0' in face_box_dict.keys():
            face_box = face_box_dict['0']
            if face_box is not None:
                faces = face_box[0]
                lm = face_box[1]
                for f in range(len(faces)):
...
# def crop_faces_from_image(in_imagefile, landmarks_path, crop_faces_out_dir, overwrite=False, frame_hops=10,
#                           buf=0.10, clean_up=True):
#     id = os.path.splitext(os.path.basename(in_imagefile))[0]
...
#     if clean_up and os.path.isfile(in_imagefile):
#         os.remove(in_imagefile)
    ...
    try:
        ...
        for j, crop in enumerate(crops):
            cv2.imwrite(os.path.join(
                out_dir, "{}_{}.png".format(in_image_id, j)), crop)
...
#     extract_landmarks_from_image_batch(input_filepath_list, landmarks_path)
...
#     input_filepath_list = get_dfdc_training_image_filepaths(data_path_root)
...
#     extract_landmarks_from_images_batch(
#         input_images_dir, landmarks_file, batch_size=32)
...
#     extract_landmarks_from_images_batch(
#         input_images_dir, landmarks_file, batch_size=128)
...
def extract_landmarks_for_datasets():
    ...
    print(f'Extracting landmarks from Celeb-df-v2 real data')
    data_path = ConfigParser.getInstance().get_celeb_df_v2_real_path()
    input_image_list = glob(data_path + '/*')  # Ubah dari video ke gambar
    extract_landmarks_from_images_batch(
        input_image_list, landmarks_path)  # Ubah dari video ke gambar
    ...
    print(f'Extracting landmarks from DFDC train data')
    data_path_root = ConfigParser.getInstance().get_dfdc_train_data_path()
    input_image_list = get_dfdc_training_image_filepaths(
        data_path_root)  # Ubah dari video ke gambar
    landmarks_path = ConfigParser.getInstance().get_dfdc_landmarks_train_path()
    extract_landmarks_from_images_batch(
        input_image_list, landmarks_path)  # Ubah dari video ke gambar
...
#     data_path = ConfigParser.getInstance().get_celeb_df_v2_real_path()
#     input_filepath_list = glob(data_path + '/*')
#     crop_faces_from_image_batch(
#         input_filepath_list, landmarks_path, crops_path)
...
def crop_faces_for_datasets():
    ...
    input_image_list = glob(data_path + '/*')  # Ubah dari video ke gambar
    crop_faces_from_image_batch(
        input_image_list, landmarks_path, crops_path)  # Ubah dari video ke gambar
    ...
    crops_path = ConfigParser.getInstance().get_fdf_crops_path()
    crop_faces_from_image_batch(
        input_images_dir, landmarks_file, crops_path)  # Ubah dari video ke gambar
