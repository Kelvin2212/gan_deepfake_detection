...
def predict_deepfake(input_image, df_method, debug=False, verbose=False):
    ...
    vid = os.path.basename(input_image)[:-4]
    ...
    if verbose:
        print(f'Extracting faces from the image')
    # Generate JSON file with location of faces
    extract_landmarks_from_image(input_image, output_path, overwrite=True)
    # Crop faces from the image using the JSON file created earlier
    crop_faces_from_image(input_image, output_path,
...
def individual_test():
    ...
    fake_prob, real_prob, pred = predict_deepfake(
        args.input_image, args.method, debug=debug, verbose=verbose)
    if pred is None:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='DeepFakes detection App. \n Use demo mode or provide input_imagefile and method')
